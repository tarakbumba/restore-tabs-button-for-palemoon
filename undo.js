...
var undoClosedButt = {
  ...
  populateUndoSubmenu: function (aNode) {
    ...
    if (undoClosedButt.session.getClosedTabCount() === 0) {
      var noHistory = document.getElementById('bundle_restoretabsbutton')
        .getString('noHistory');
    ...
    function addClear() {
      var clearHistoryLabel = document.getElementById('bundle_restoretabsbutton')
    ...
    if (
      gPrefService.getBoolPref('extensions.restoretabsbutton.showOpenAll') ||
      gPrefService.getBoolPref('extensions.restoretabsbutton.clearHistory')
    ) {
      if (gPrefService.getBoolPref('extensions.restoretabsbutton.onTop')) {
        if (gPrefService.getBoolPref('extensions.restoretabsbutton.clearHistory')) {
          addClear();
        }
        if (gPrefService.getBoolPref('extensions.restoretabsbutton.showOpenAll')) {
          addRestore();
        }
        addSeparator();
        addItems();
  ...
  onCommand: function () {
    if (gPrefService.getBoolPref('extensions.restoretabsbutton.openMenu')) {
  ...
  welcome: function () {
    ...
    try {
      version = (
        ...
        navigator.userAgent.match(/PaleMoon\/([\d\.]*)/) ||
        navigator.userAgent.match(/Goanna\/([\d\.]*)/)
      )[1];
    } catch (e) {}
    //FF < 4.*
...
function UCT_init() {
  ...
  if (gPrefService.getBoolPref('extensions.restoretabsbutton.firstRun')) {
    ...
    gPrefService.setBoolPref('extensions.restoretabsbutton.firstRun', false);
  }
  // Set hotkey
  var modifier = gPrefService.getCharPref('extensions.restoretabsbutton.modifier');
  var key = gPrefService.getCharPref('extensions.restoretabsbutton.key');
  if (key && modifier) {
  ...
  try {
    if (gPrefService.getBoolPref('extensions.restoretabsbutton.oldAppearance')) {
  ...
  document.getElementById('contentAreaContextMenu').addEventListener('popupshowing', function () {
    ...
    someConditions = someConditions &&
      !gPrefService.getBoolPref('extensions.restoretabsbutton.removecontextentry');
